...
export const store_carousel_old = pgTable("store_carousel_old", {
  ...
  // storeId stays uuid because createStore._id is uuid (not changing store _id)
...
export const carousel_items_old = pgTable("carousel_items_old", {
  ...
  // FK to store_carousel_old._id which is now bigint
  storeCarouselId: bigint("store_carousel_id", { mode: "number" })
    .notNull()
  ...
  discriptionBackgroundColor: varchar("discription_background_color", {
    length: 100,
  }),
  ...
  // productId will be bigint (product _id will also be bigint)
  ...
  storeCarouselIdx: index("carousel_items_old_store_carousel_idx").on(
    table.storeCarouselId
  ),
}));
