...
class StoreCarouselOldService {
  ...
  async createCarousel(
    ...
    // storeId is still uuid (createStore._id is uuid)
    if (!isUUID(storeId)) {
    ...
    if (!storeCarouselDoc)
      throw new ApiError(
        500,
        "Failed to create or retrieve store carousel document"
      );
    ...
    for (const meta of carouselsData) {
      ...
      if (!file) {
        errors.push({
          index,
          error: `No image file for carousel index ${index}`,
        });
        continue;
      }
      ...
      newCarousels.push({
        ...
        ...(fields.descriptionColor && {
          descriptionColor: fields.descriptionColor,
        }),
        ...(fields.discriptionBackgroundColor && {
          discriptionBackgroundColor: fields.discriptionBackgroundColor,
        }),
        ...(fields.buttonText && { buttonText: fields.buttonText }),
        ...(fields.buttonTextColor && {
          buttonTextColor: fields.buttonTextColor,
        }),
        ...(fields.buttonHoverTextColor && {
          buttonHoverTextColor: fields.buttonHoverTextColor,
        }),
        ...(fields.buttonBackground && {
          buttonBackground: fields.buttonBackground,
        }),
        ...(fields.buttonHoverBackground && {
          buttonHoverBackground: fields.buttonHoverBackground,
        }),
        ...(fields.buttonShadowColor && {
          buttonShadowColor: fields.buttonShadowColor,
        }),
        ...(fields.buttonBorderColor && {
          buttonBorderColor: fields.buttonBorderColor,
        }),
        ...(fields.category && { category: fields.category }),
        // productId is now number (bigint) — parse it
        ...(fields.productId && {
          productId: Number(fields.productId),
        }),
        ...(fields.fontFamily?.length && { fontFamily: fields.fontFamily }),
        ...(fields.buttonShadow !== undefined && {
          buttonShadow: fields.buttonShadow,
        }),
        ...(fields.buttonBorder !== undefined && {
          buttonBorder: fields.buttonBorder,
        }),
    ...
    const savedCarousels =
      newCarousels.length > 0
        ? await storeCarouselOldRepository.createCarouselItems(
            // storeCarouselDoc._id is now number (bigint)
  ...
  async updateCarousel(
    storeId: string,
    carouselId: string, // comes as string from params, will parse to number
    fields: any,
    file?: any
  ) {
    ...
    // carouselId is now bigint — validate it's a valid number
    const carouselIdNum = Number(carouselId);
    // if (!Number.isInteger(carouselIdNum) || carouselIdNum <= 0) {
    //   throw new ApiError(400, "Invalid carousel ID format");
    // }

    if(!isValidId(carouselIdNum)) throw new ApiError(400, "Invalid carousel ID format");

    const existing = await storeCarouselOldRepository.findCarouselItem(
      storeId,
      carouselIdNum
    );
    if (!existing)
    ...
    if (fields.titleColor !== undefined)
      updateFields.titleColor = fields.titleColor;
    if (fields.tileBackground !== undefined)
      updateFields.tileBackground = fields.tileBackground;
    if (fields.imageAlt !== undefined) updateFields.imageAlt = fields.imageAlt;
    // productId is now number
    if (fields.productId !== undefined)
      updateFields.productId = Number(fields.productId);
    if (imageUrl) updateFields.images = imageUrl;

    if (Object.keys(updateFields).length === 0)
      throw new ApiError(400, "No fields provided to update");

    return await storeCarouselOldRepository.updateCarouselItem(
      carouselIdNum,
      updateFields
    );
  }
  ...
  async deleteCarousel(storeId: string, carouselId: number) {
    ...
    // carouselId is now bigint — parse to number
    // const carouselIdNum = Number(carouselId);
    // if (!Number.isInteger(carouselIdNum) || carouselIdNum <= 0) {
    //   throw new ApiError(400, "Invalid carousel ID format");
    // }

    if(!isValidId(carouselId)) throw new ApiError(400, "Invalid carousel ID format");
